...
class Order_Manage_Screen extends StatefulWidget {
  const Order_Manage_Screen({Key? key, this.id, this.isOrder}) : super(key: key);
  final id;
  final isOrder;

  @override
  State<Order_Manage_Screen> createState() => _Order_Manage_ScreenState();
}

class _Order_Manage_ScreenState extends State<Order_Manage_Screen> {
  ...
  void initState() {
    ...
    setupDialer();
    if(widget.id!=null){
      storage.write('isOrdermen', widget.id.toString());
    }
    if(widget.isOrder!=true){
      storage.write('isOrder', widget.isOrder);
    }
    orderDetails();
  }
  ...
  void orderDetails() async {
    ...
    // try {
      print('---->${widget.id}');
      final url = '${Url}orders/${widget.id}';
      print(url);
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (data['success'] == true) {
          setState(() {
            ...
            Navigator.pushReplacement(context, NoBlinkPageRoute(builder: (context)=>  Order_Manage_Screen(id: storage.read('isOrdermen'),isOrder: storage.remove('isOrder'),)));
          });
        } else {
          setState(() {
            isLoading = false;
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: Wid_Con.App_Bar(
            ...
            titel: 'ORDERS | ORDERS MANAGEMENT',
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        if( foodList['order_type'] == "Dine In")
                                        Column(
                                        ...
                                        foodList['payment']['method'] == "Cashfree" ?
                                        Text(
                                           'Online',
                                          style: TextStyle(
                                            fontSize: 10,
